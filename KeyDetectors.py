...
def LinuxKeyDetection():
    ...
    variables meet the standards shown below, code is a value listed in Dictionaries.KEY_CODES, and
    global_variables.input.key has already been read, then global_variables.input.key has the corresponding key written
    to it and global_variables.input.write_ready is set to False. Note, I didn't write all of this
    myself, and took quite a bit here from StackOverflow.
    :return: Nothing notable.
    '''
    try:
        keyboardInput = open(global_config.event_file_location, "rb")
...
def ChownEventFile():
    os.system("sudo chown $USER:$USER " + global_config.event_file_location)
